...
router.post('/', authMiddleware, async (req, res) => {
  try {
    ...
    const id = uuidv4();
    await db.runAsync('INSERT INTO messages (id, sender_id, receiver_id, body, type) VALUES (?, ?, ?, ?, ?)', [id, req.user.id, receiver_id, body, type || 'text']);
...
router.get('/unread', authMiddleware, async (req, res) => {
  try {
    const result = await db.getAsync('SELECT COUNT(*) as count FROM messages WHERE receiver_id = ? AND read = 0', [req.user.id]);
    res.json(result);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
